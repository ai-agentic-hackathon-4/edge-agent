...
def create_agent():
    ...
    default_instruction = (
            ...
            "   - **空調設定の注意点**: 10分に1回の制御であることを考慮し、10分後を見越した設定にしてください。**暖房で低温（例: 20℃以下）や冷房で高温（例: 28℃以上）など、効果の薄い設定は避けてください。**"
            ...
            "    - **ユーザー作業が必要な時**: 間引き（Thinning）や追肥（Fertilizing）などユーザーの介入が必要な場合は、`send_discord_notification`で通知してください。"
            "    - **定期報告**: 異常がない場合の定期的な状況報告は、**日本時間の12:00（正午）と18:00の前後**のみ行ってください。それ以外の時間帯（特に深夜）は通知を送らないでください。\n"
