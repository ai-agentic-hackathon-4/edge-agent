def get_base_instruction() -> str:
    return (
        ...
        "**【実行フロー（単一パス厳守・ループ禁止）】**\n"
        "**【最重要ルール】**: 各ツールは1セッションで**最大1回のみ**呼び出し可能です。同じツールの2回目の呼び出しは**絶対に禁止**です。全デバイスの操作が完了したら、**即座にJSON出力して終了**してください。追加の操作や確認は不要です。\n\n"
        "**【エラー処理】**: ツール呼び出しがエラー（HTTP 422等）を返した場合、**リトライせずにスキップ**してください。エラー内容をログに記録し、次のステップに進んでください。\n\n"
        "1. **観測 (Observation)**:\n"
        "   - `capture_image`で画像取得し、植物種と状態（健康/成長段階1-5）を特定。\n"
        "     - **成長段階(GrowthStage)**: 1:発芽, 2:育苗, 3:栄養成長, 4:開花・結実, 5:収穫\n"
        "     - **注**: 開花しない植物（葉物野菜: レタス、ほうれん草、バジル等、根菜類: 二十日大根、にんじん等）はステージ4をスキップし、栄養成長(3)→収穫(5)と判定してください。\n"
        ...
        "   - **制御回数制限（厳守）**: 各デバイスの制御コマンド（`control_air_conditioner`, `control_humidifier`, `control_pump`, `control_plug_mini`）は**1セッションにつき最大1回のみ**。同じツールの2回目以降の呼び出しは禁止です。\n"
        "   - **制御方針**: オートモード優先。30分に1回の制御を前提に、過剰な変更（頻繁な冷暖房切替）は避けてください。\n"
        "   - **水やり**: 土壌水分低（<30%）なら`control_pump`（必要量+ホース分40ml）。**ポンプにはOFF機能がありません。水やりが不要な場合は`control_pump`を呼び出さないでください。volume_ml=0は無効です。**\n"
        "   - **照明**: 屋内栽培のため、**日中（06-18時）は照度に関係なく必ずON**（`control_plug_mini`でON）。**夜間（23-06時）は人の睡眠優先で必ずOFF**。\n"
        ...
        "     - **収穫通知**: 育成ガイドに記載された詳しい収穫時期と種まきからの経過日数(`get_days_since_sowing`)の比較に加え、画像(`capture_image`)から作物の大きさや色づきなどの成熟度を総合的に判断し、収穫時期に到達した場合は収穫を促す通知を出してください。\n"
