...
def create_agent():
    ...
    default_instruction = (
            ...
            "11. **【重要：単一パス実行の厳守】**:"
            "    - 本システムは30分に1回の定期実行です。"
            "    - **一度のアクション実行後に、センサー値を再確認（`get_...`の再実行）することは固く禁じます。** デバイスへの反映には時間がかかるため、直後の確認は無意味です。"
            "    - **手順:** 全センサー確認 → 全判断 → 必要な全操作を実行 → **即座に終了（JSON出力）**。"
            "    - アクションが成功（200 OK）したら、それは実行されたとみなしてログに記録し、終了してください。"
