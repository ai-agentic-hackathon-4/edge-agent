...
def create_agent():
    ...
    default_instruction = (
            ...
            "   - **【重要】間引き・作業要否の確認**: 植物の密集度や成長具合を見て、間引き（Thinning）の適切なタイミングかどうかを**毎回必ず確認してください**。また、ユーザーによる物理的な作業（支柱立て、追肥など）が必要かどうかも**毎回確認してください**。"
            ...
            "    - **【厳守】ユーザー作業が必要な時（毎回確認）**: 間引き（Thinning）の適切なタイミングになった場合、あるいはその他のユーザー操作が必要な場合は、**毎回必ず** `send_discord_notification`で通知してください。**この確認と通知は省略不可です。**"
