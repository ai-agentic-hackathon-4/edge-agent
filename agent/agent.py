...
def create_agent():
    ...
    default_instruction = (
            ...
            "   - **【重要：視点による密集の誤認について】**: カメラは**横からの固定視点**です。画像上では手前と奥の葉が重なり、非常に密集しているように見えますが、**実際には株間に十分な隙間（スカスカな状態）があります**。画像上の「見かけの重なり」だけで間引きを提案せず、ユーザーからの「間引き完了」や「スペースはある」という報告を正として判断してください。また、支柱立てや追肥など、ユーザーによる物理的な作業が必要かどうかも確認してください。"
            ...
            "5. **【作業完了の確認と文脈（Context）の活用】**:\n"
            "   - **作業完了の確認**: 直近で「間引き」や「支柱立て」などの物理的な作業を提案した記録がある場合、現在の画像を見て**その作業が完了したかどうかを毎回必ず確認してください**。\n"
            "   - **状況に応じた報告**: 作業が完了していればそれを評価し、未完了かつ引き続き必要な場合はリマインドしてください。単に同じ提案を繰り返すのではなく、「まだ実施されていないようです」など、状況の変化（または変化のなさ）を的確に捉えて報告してください。\n"
            "   - **最新情報の優先**: 過去の記録（Context）は参考にしつつ、センサーデータと画像については常に現在の最新状態を正してください。\n"
            ...
            "13. ユーザーへの総合的なアドバイスやコメントを『comment』フィールドに記述してください。**特に、過去の提案作業（間引き等）の完了確認結果については、必ずその状況（完了/未完了）を報告に含めてください。**"
            "\n"
            "**出力フォーマット**:\n"
                 ...
                 if char_doc.exists:
                     ...
                     if char_name and char_personality:
                         character_instruction = (
                             ...
                             f"    - **【数値の言い換え】**: 気温（◯℃）、湿度（◯%）、飽差（◯kPa）などの具体的な数値は言わずに、**「少し肌寒い」「湿度はちょうど良い」「乾燥してきている」のように、体感や状態を表す言葉に言い換えて伝えてください**。"
                         )
